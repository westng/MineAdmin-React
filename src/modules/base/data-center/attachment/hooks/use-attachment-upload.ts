...
export function useAttachmentUpload(onUploaded: () => void) {
  ...
  const startUpload = useCallback(async (id?: string) => {
    if (controllerRef.current) return
    if (!hasAuth('dataCenter:attachment:upload')) { toast('暂无上传权限，请联系管理员', 'destructive'); return }
    const queued = selection.files.filter(item => (!statuses[item.id] || statuses[item.id].status === 'error') && (!id || item.id === id))
    ...
    const update = (id: string, change: Partial<UploadStatus>) => {
      if (!controller.signal.aborted) setStatuses(current => ({ ...current, [id]: { ...(current[id] ?? queuedStatus), ...change } }))
    }
    try {
      for (const item of queued) {
        if (controller.signal.aborted) break
        if (!hasAuth('dataCenter:attachment:upload')) { toast('上传权限已失效，请联系管理员', 'destructive'); break }
        update(item.id, { status: 'uploading', progress: null, error: undefined })
        try {
          if (!(item.file instanceof File)) throw new Error('文件不可用，请重新选择')
          const response = await upload(item.file, {
            timeout: 120_000,
            signal: controller.signal,
            onUploadProgress: event => update(item.id, { progress: event.total ? Math.min(100, Math.round(event.loaded / event.total * 100)) : null }),
          })
          if (response.data.code !== 200) throw new Error(response.data.message || '上传失败')
          if (!response.data.data?.id) throw new Error('上传响应缺少附件记录，请刷新列表确认后再重试')
          update(item.id, { status: 'success', progress: 100 })
          successCount += 1
        } catch (error) {
          if (controller.signal.aborted) break
          update(item.id, { status: 'error', error: attachmentError(error, '上传失败，请重试') })
          failedCount += 1
      ...
      if (!controller.signal.aborted) {
        ...
        if (successCount || failedCount) toast(`上传成功 ${successCount} 个${failedCount ? `，失败 ${failedCount} 个` : ''}`, failedCount ? 'destructive' : 'success')
      }
    } finally {
