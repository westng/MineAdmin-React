  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (response.data.code !== 200) throw new Error(response.data.message || tx('附件加载失败'))
        const data = response.data.data
        if (!data || !Array.isArray(data.list) || !Number.isFinite(data.total) || data.total < 0)
          throw new Error(tx('附件列表响应异常，请重试'))
        ...
        if (!controller.signal.aborted)
          setResult({ request, rows: [], total: 0, error: attachmentError(error, tx('附件加载失败，请稍后重试')) })
      }
    }
    void load()
