...
import {
  ...
  save,
  type RoleVo,
} from '@/modules/base/permission/role/api/role'
...
export default function PermissionRolePageView() {
  ...
  async function submitForm() {
    ...
    try {
      ...
      setFormOpen(false)
      toast(form.id ? tx('角色更新成功') : tx('角色创建成功'), 'success')
      await refreshRoles()
    } catch (error) {
      toast(error instanceof Error ? error.message : tx('角色保存失败'), 'destructive')
    }
  }
  ...
  async function confirmRemoveRoles() {
    ...
    try {
      ...
      if (response.data.code !== 200) throw new Error(responseMessage(response))
      toast(tx('角色删除成功'), 'success')
      setConfirmDeleteIds([])
      await refreshRoles()
    } catch (error) {
      toast(error instanceof Error ? error.message : tx('角色删除失败'), 'destructive')
    }
  }
  ...
  async function openPermissions(role: RoleVo) {
    ...
    try {
      ...
      if (requestId === permissionRequestRef.current)
        toast(error instanceof Error ? error.message : tx('权限数据加载失败'), 'destructive')
    } finally {
  ...
  async function savePermissions() {
    ...
    try {
      ...
      if (response.data.code !== 200) throw new Error(responseMessage(response))
      closePermissions()
      toast(tx('角色权限更新成功'), 'success')
    } catch (error) {
      if (requestId !== permissionRequestRef.current) return
      toast(error instanceof Error ? error.message : tx('角色权限更新失败'), 'destructive')
    } finally {
      if (requestId === permissionRequestRef.current) {
        permissionSavingRef.current = false
        setPermissionSaving(false)
    ...
    <>
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader>
            <DialogTitle>{form.id ? tx('编辑角色') : tx('新增角色')}</DialogTitle>
            <DialogDescription>{tx('角色编码用于权限识别，保存后可继续配置菜单权限。')}</DialogDescription>
          </DialogHeader>
          <FieldGroup className="grid gap-4 md:grid-cols-2">
            <Field>
              <FieldLabel>{tx('角色名称')}</FieldLabel>
              <Input
                value={form.name || ''}
            ...
            <Field>
              ...
              <Select
                ...
                onValueChange={value => setForm(current => ({ ...current, status: Number(value) as 1 | 2 }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
          ...
          <DialogFooter>
            ...
            <PermissionGate permission={form.id ? 'permission:role:update' : 'permission:role:save'}>
              <Button onClick={() => void submitForm()}>{tx('保存')}</Button>
            </PermissionGate>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      >
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>{tx('配置菜单权限')}</DialogTitle>
            <DialogDescription>
              {permissionRole?.name || tx('当前角色')} {tx('可以访问的菜单。')}
            </DialogDescription>
          </DialogHeader>
      ...
      <ConfirmDialog
        ...
        title={tx('删除角色')}
        description={tx('确认删除 {0} 个角色吗？', { '0': confirmDeleteIds.length })}
        onClose={() => setConfirmDeleteIds([])}
        onConfirm={confirmRemoveRoles}
      />
    </>
  )
}
