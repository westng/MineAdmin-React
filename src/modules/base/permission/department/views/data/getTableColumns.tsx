...
export function getDepartmentTableColumns({ rows, selectedIds, collapsedIds, loading, canManageLeaders, canManagePositions, onSelectionChange, onToggle, onCreate, onEdit, onDetails, onLeaders, onPositions, onDelete }: DepartmentColumnActions): MaProTableColumns<DepartmentRow>[] {
  ...
  const actions: MaProTableOperationAction<DepartmentRow>[] = [
    { name: 'create-child', text: '子部门', icon: <Plus aria-hidden="true" />, onClick: ({ row }) => onCreate(row.department) },
    { name: 'leaders', text: '设置负责人', icon: <UserRoundCog aria-hidden="true" />, show: () => canManageLeaders, disabled: ({ row }) => loading || !row.department.id, onClick: ({ row }) => onLeaders(row.department) },
    { name: 'positions', text: '管理岗位', icon: <BriefcaseBusiness aria-hidden="true" />, show: () => canManagePositions, disabled: ({ row }) => loading || !row.department.id, onClick: ({ row }) => onPositions(row.department) },
    { name: 'details', text: '详情', icon: <Eye aria-hidden="true" />, onClick: ({ row }) => onDetails(row.department) },
    { name: 'edit', text: '编辑', icon: <Pencil aria-hidden="true" />, onClick: ({ row }) => onEdit(row.department) },
    { name: 'delete', text: '删除', icon: <Trash2 aria-hidden="true" />, variant: 'destructive', onClick: ({ row }) => { if (row.department.id) void onDelete([row.department.id]) } },
  ]
  return [
    ...
    { type: 'operation', label: '操作', align: 'right', width: 320 + (canManageLeaders ? 120 : 0) + (canManagePositions ? 110 : 0), operationConfigure: { type: 'auto', actions } },
  ]
}
