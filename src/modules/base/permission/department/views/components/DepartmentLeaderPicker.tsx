...
export function DepartmentLeaderPicker({ departmentName, busy, onClose, onAdd }: Props) {
  ...
  const request = useCallback(async (params: Record<string, unknown>) => {
    ...
    setSelectedIds([])
    try {
      if (!hasAuth('permission:user:index')) throw new Error(tx('暂无查看用户权限，请联系管理员'))
      ...
      if (response.data.code !== 200) throw new Error(response.data.message || tx('用户列表加载失败'))
      return response
    } catch (error) {
      throw new Error(departmentErrorMessage(error, tx('用户列表加载失败，请重试')), { cause: error })
    } finally {
  ...
  return (
    ...
    >
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-3xl" showCloseButton={!busy}>
        <DialogHeader>
          <DialogTitle>{tx('添加负责人')}</DialogTitle>
          <DialogDescription>
            {tx('为“')}
            {departmentName}
            {tx('”选择已启用的用户，可在当前页多选后添加。')}
          </DialogDescription>
        </DialogHeader>
        <MaProTable<UserVo>
          ref={tableRef}
          ...
          options={{
            ...
            tableOptions: { rowKey: 'id', emptyText: tx('暂无符合条件的启用用户') },
          }}
          toolbarLeft={
