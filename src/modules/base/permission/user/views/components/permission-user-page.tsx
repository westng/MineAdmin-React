...
import { PermissionGate, hasAuth } from '@/hooks/framework/use-permission'
...
import {
  ...
  setUserRole,
  type UserVo,
} from '@/modules/base/permission/user/api/user'
...
export default function PermissionUserPageView() {
  ...
  const getStatusLabel = useCallback(
    (value: unknown) => String(dictionary('system-status', String(value)) || tx('未知')),
    [dictionary],
  )
  const getUserTypeLabel = useCallback(
    (value: unknown) => String(dictionary('base-userType', String(value)) || tx('普通用户')),
    [dictionary],
  )
  ...
  useEffect(() => {
    let active = true
    const normalizeDepartments = (items: DepartmentVo[]): DepartmentOption[] =>
      items.flatMap(item => {
      ...
      .then(([departmentResponse, positionResponse]) => {
        ...
        setPositions(
          positionList.flatMap(position =>
            position.id && position.name ? [{ id: position.id, dept_id: position.dept_id, name: position.name }] : [],
          ),
        )
      })
      .catch(() => {
  ...
  const openEdit = useCallback((user: UserVo) => {
    setForm({
      ...
      policy_func:
        user.policy?.policy_type === 'CUSTOM_FUNC' && user.policy.value?.[0] != null
          ? String(user.policy.value[0])
          : '',
    })
    setFormOpen(true)
  }, [])
  ...
  const submitForm = useCallback(
    async (values: UserForm) => {
      ...
      try {
        ...
        setFormOpen(false)
        setNotice(id ? tx('用户更新成功') : tx('用户创建成功'))
        await refreshUsers()
      } catch (error) {
        setNotice(error instanceof Error ? error.message : tx('用户保存失败'))
      }
    },
    [refreshUsers],
  )
  ...
  const confirmRemoveUsers = useCallback(async () => {
    ...
    try {
      ...
      setNotice(error instanceof Error ? error.message : tx('用户删除失败'))
    }
  ...
  const openRoles = useCallback(async (user: UserVo) => {
    ...
    try {
      ...
      if (requestId === roleRequestRef.current)
        setNotice(error instanceof Error ? error.message : tx('角色信息加载失败'))
    } finally {
  ...
  async function saveRoles() {
    ...
    try {
      ...
      if (response.data.code !== 200) throw new Error(responseMessage(response))
      closeRoles()
      setNotice(tx('用户角色更新成功'))
    } catch (error) {
      if (requestId !== roleRequestRef.current) return
      setNotice(error instanceof Error ? error.message : tx('用户角色更新失败'))
    } finally {
      if (requestId === roleRequestRef.current) {
        roleSavingRef.current = false
        setRoleSaving(false)
  ...
  const confirmInitializePassword = useCallback(async () => {
    ...
    try {
      ...
      if (response.data.code !== 200) throw new Error(responseMessage(response))
      setNotice(tx('初始密码设置成功'))
      setResetUser(null)
    } catch (error) {
      setNotice(error instanceof Error ? error.message : tx('密码重置失败'))
    }
  }, [resetUser])

  const formItems = useMemo(() => {
    void localeRevision // Rebuild translated configuration when the active locale changes.
    return getFormItems(Boolean(form.id), { departments, positions })
  }, [departments, form.id, positions, localeRevision])

  return (
    <>
      {notice && (
        <div className="flex items-center justify-between rounded-md border bg-muted/30 px-4 py-2 text-sm text-muted-foreground">
          <span>{notice}</span>
      ...
      <MaDialog
        ...
        footer={
          <>
            ...
            <PermissionGate permission={form.id ? 'permission:user:update' : 'permission:user:save'}>
              <Button onClick={() => formRef.current?.getElFormRef()?.requestSubmit()}>{tx('保存')}</Button>
            </PermissionGate>
          </>
        }
        okText={tx('保存')}
        cancelText={tx('取消')}
        onOk={() => {
          if (!hasAuth(form.id ? 'permission:user:update' : 'permission:user:save')) return false
          formRef.current?.getElFormRef()?.requestSubmit()
          return false
        }}
        showFullscreenButton
        contentClassName="max-h-[calc(100vh-2rem)] overflow-hidden sm:max-w-4xl"
        bodyClassName="min-h-0 overflow-y-auto"
      >
        <MaForm<UserForm>
          ref={formRef}
          key={`${formOpen ? 'open' : 'closed'}-${form.id ?? 'new'}`}
          modelValue={form}
          items={formItems}
          options={{ layout: 'grid', grid: { columns: 2, gap: '1.25rem' }, containerClass: 'pb-1' }}
      ...
      <Dialog
        open={roleOpen}
        onOpenChange={open => {
          if (!roleSavingRef.current) {
            if (open) setRoleOpen(true)
            else closeRoles()
          }
        }}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{tx('设置用户角色')}</DialogTitle>
            <DialogDescription>
              {roleUser?.username || tx('当前用户')} {tx('可分配的角色。')}
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-2">
            {roleLoading && <p className="text-sm text-muted-foreground">{tx('正在加载角色信息…')}</p>}
            {!roleLoading &&
              roleReady &&
              roles.map(role => (
                <label key={role.code} className="flex items-center gap-2 rounded-md border px-3 py-2 text-sm">
                  <Checkbox
                    disabled={roleSaving}
                    checked={Boolean(role.code && roleCodes.includes(role.code))}
                    onCheckedChange={checked =>
                      setRoleCodes(current =>
                        checked ? [...current, role.code as string] : current.filter(code => code !== role.code),
                      )
                    }
                  />
                  <span>
            ...
            {!roleLoading && !roleReady && (
              <p className="text-sm text-destructive">{tx('角色信息加载失败，请关闭后重试。')}</p>
            )}
            {!roleLoading && roleReady && !roles.length && (
              <p className="text-sm text-muted-foreground">{tx('暂无可分配角色。')}</p>
            )}
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={closeRoles} disabled={roleSaving}>
              {tx('取消')}
            </Button>
            <PermissionGate permission="permission:user:setRole">
              <Button
                disabled={roleLoading || roleSaving || !roleReady || !roleUser?.id}
                onClick={() => void saveRoles()}
              >
                {roleSaving ? tx('保存中…') : tx('保存角色')}
              </Button>
            </PermissionGate>
          </DialogFooter>
        </DialogContent>
      ...
      <ConfirmDialog
        ...
        title={tx('删除用户')}
        description={tx('确认删除 {0} 个用户吗？', { '0': confirmDeleteIds.length })}
      ...
      <ConfirmDialog
        ...
        title={tx('重置用户密码')}
        description={tx('确认将 {0} 的密码重置为初始密码吗？', { '0': resetUser?.username || '该用户' })}
        onClose={() => setResetUser(null)}
        onConfirm={confirmInitializePassword}
      />
    </>
  )
}
