...
import { MaProTable, type MaProTableApi, type MaProTableColumns, type MaProTableExpose, type MaProTableModel, type MaProTableOptions } from '@/components/ma-pro-table'
...
export function LogProTable<T extends MaProTableModel & { id: number }>({ tableRef, title, description, listPermission, api, searchItems, getTableColumns, canDelete, deleting, selectedIds, onSelectionChange, onDetail, onDelete }: Props<T>) {
  const { toast } = useToast()
  const [requestPending, setRequestPending] = useState(true)
  const requestSequence = useRef(0)
  const busy = deleting || requestPending
  const columns = useMemo(() => getTableColumns({ canDelete, busy, onDetail, onDelete }), [busy, canDelete, getTableColumns, onDelete, onDetail])
  ...
  }), [description, listPermission, request, title, toast])
