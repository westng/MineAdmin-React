...
import { menuApi, type MenuVo } from '@/modules/base/api'
...
function createMenuTreeData(menus: MenuVo[]) {
  ...
  function addMenu(menu: MenuVo, path: number[]): string {
    ...
    const children = (menu.children || []).filter(child => child.meta?.hidden !== true).map((child, index) => addMenu(child, [...path, index]))
    items[itemId] = { label: getMenuLabel(menu), menu, type: getMenuType(menu), children }
    return itemId
  }

  items[rootItemId].children = menus.filter(menu => menu.meta?.hidden !== true).map((menu, index) => addMenu(menu, [index]))
...
export default function PermissionMenuPage() {
