...
type SessionRequest = InternalAxiosRequestConfig & { _retry?: boolean; _sessionVersion?: number; _managedToken?: string }

async function retryAuthenticatedRequest(config: SessionRequest) {
  const state = useUserStore.getState()
  if (config._retry || !config._managedToken || config._sessionVersion !== state.sessionVersion
    || /\/admin\/passport\/(login|logout|refresh)(?:[/?]|$)/.test(config.url || '')) return null
  config._retry = true
...
http.interceptors.request.use(async config => {
  const { token, language, sessionVersion } = useUserStore.getState()
  ...
  if (sessionConfig._sessionVersion !== undefined && sessionConfig._sessionVersion !== useUserStore.getState().sessionVersion) throw new axios.CanceledError('登录会话已变化')
  return config
})

http.interceptors.response.use(async response => {
  const config = response.config as SessionRequest
  if (config._sessionVersion !== undefined && config._sessionVersion !== useUserStore.getState().sessionVersion) {
    return Promise.reject(new axios.CanceledError('登录会话已变化'))
  }
  await usePluginStore.getState().callHooks('networkResponse', response)
  if (config._sessionVersion !== undefined && config._sessionVersion !== useUserStore.getState().sessionVersion) return Promise.reject(new axios.CanceledError('登录会话已变化'))
