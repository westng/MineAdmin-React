import { queryClient, queryKeys } from '@/provider/query/client'
...
export const useNavigationStore = create<MenuState>((set, get) => ({
  menus: [],
  loading: false,
  initialized: false,
  error: null,
  ...
  refreshMenus: async () => {
    ...
    try {
      const response = await queryClient.fetchQuery({
        queryKey: queryKeys.resource(useSessionStore.getState().sessionVersion, 'auth', 'menus'),
      ...
      routeRegistry.setMenus(menus)
      const snapshot = routeRegistry.getSnapshot()
      const routes = snapshot.menuRoutes.filter(route => snapshot.menus.includes(route.menu))
  ...
  refreshRoles: async () => {
    ...
    try {
      const response = await queryClient.fetchQuery({
        queryKey: queryKeys.resource(useSessionStore.getState().sessionVersion, 'auth', 'roles'),
