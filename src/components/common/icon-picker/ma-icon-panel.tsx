...
function IconOption({ name, selected, disabled, onSelect }: {
  name: string
  selected: boolean
  disabled?: boolean
  ...
  const label = icon.status === 'error'
    ? `${name}（加载失败或图标已失效，请检查网络或刷新页面后重试）`
    : icon.status === 'loading' ? `${name}（加载中）` : name
  ...
  return (
    <span className="min-w-0" title={label}>
      <Button
        type="button"
        variant="ghost"
        className={cn('h-12 w-full min-w-0 p-2', selected && 'bg-primary/10 text-primary ring-2 ring-primary ring-inset')}
        aria-label={label}
        aria-pressed={selected}
        disabled={disabled || icon.status !== 'ready'}
        onClick={() => onSelect(name)}
      >
        {icon.status === 'ready' && ('data' in icon
          ? <Icon icon={icon.data} className="size-[26px]" aria-hidden="true" />
          : <img src={icon.src} className="size-[26px] object-contain" alt="" />)}
        {icon.status === 'loading' && <LoaderCircle className="size-5 animate-spin motion-reduce:animate-none" aria-hidden="true" />}
...
export function MaIconPanel({ value, onSelect, pageSize = 70, disabled }: MaIconPanelProps) {
  ...
  const [category, setCategory] = useState(() => collections.find(item =>
    item.icons.some(name => getIconValue(item.prefix, name) === selected),
  )?.prefix ?? collections[0].prefix)
    ...
    >
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col gap-3">
        <InputGroup className="shrink-0">
          ...
          {keywords && (
            <InputGroupAddon align="inline-end">
              <InputGroupButton aria-label="清除图标搜索" onClick={() => search('')}><X aria-hidden="true" /></InputGroupButton>
            </InputGroupAddon>
          )}
        </InputGroup>
        {collections.map(item => (
          <TabsContent key={item.prefix === category ? `${category}:${keywords}:${currentPage}` : item.prefix} value={item.prefix} className="min-h-0 overflow-y-auto p-1">
            {item.prefix === category && (pageIcons.length ? (
              <div className="grid grid-cols-5 gap-1 sm:grid-cols-8 md:grid-cols-10">
                {pageIcons.map(name => {
                  const icon = getIconValue(category, name)
                  return <IconOption key={icon} name={icon} selected={selected === icon} disabled={disabled} onSelect={onSelect} />
                })}
              </div>
            ) : (
              <div className="flex h-full min-h-24 items-center justify-center px-4 text-center text-muted-foreground" role="status">
        ...
        <div className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-t pt-3">
          <span className="text-xs text-muted-foreground" role="status">共 {filteredIcons.length} 个图标 · {currentPage} / {pageCount} 页</span>
          <nav aria-label="图标分页" className="flex items-center gap-1">
            <Button variant="outline" size="icon-sm" aria-label="上一页" disabled={currentPage === 1} onClick={() => setPage(currentPage - 1)}><ChevronLeft aria-hidden="true" /></Button>
            {firstPage > 1 && <Button variant="ghost" size="icon-sm" aria-label="第 1 页" onClick={() => setPage(1)}>1</Button>}
            {firstPage > 2 && <span className="text-muted-foreground" aria-hidden="true">…</span>}
            {pages.map(number => (
              <Button key={number} variant={number === currentPage ? 'default' : 'ghost'} size="icon-sm" aria-label={`第 ${number} 页`} aria-current={number === currentPage ? 'page' : undefined} disabled={!filteredIcons.length} onClick={() => setPage(number)}>{number}</Button>
            ))}
            {firstPage + pages.length < pageCount && <span className="text-muted-foreground" aria-hidden="true">…</span>}
            {firstPage + pages.length <= pageCount && <Button variant="ghost" size="icon-sm" aria-label={`第 ${pageCount} 页`} onClick={() => setPage(pageCount)}>{pageCount}</Button>}
            <Button variant="outline" size="icon-sm" aria-label="下一页" disabled={currentPage === pageCount} onClick={() => setPage(currentPage + 1)}><ChevronRight aria-hidden="true" /></Button>
          </nav>
        </div>
      </div>
    </TabsPrimitive.Root>
