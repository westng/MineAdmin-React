...
export function MaIconPicker({
  value = '', onChange, className, placeholder = '点击后面按钮选择图标',
  pageSize = 70, disabled = false, readOnly = false, id, ...inputProps
}: MaIconPickerProps) {
    ...
    <Dialog open={open} onOpenChange={changeOpen}>
      <div className={cn('flex w-full min-w-0 items-center gap-2', className)} role="group" aria-label="图标选择">
        <InputGroup className="min-w-0 flex-1">
          {value && <InputGroupAddon><MaIcon name={value} className="size-5" /></InputGroupAddon>}
          <InputGroupInput
            {...inputProps}
            id={id ?? generatedId}
      ...
      <DialogContent className="flex max-h-[calc(100dvh-2rem)] flex-col overflow-hidden sm:max-w-[800px]">
        ...
        {Panel ? (
          ...
          <div className="flex h-[min(500px,calc(100dvh-8rem))] flex-col items-center justify-center gap-3 text-muted-foreground" role="status">
            {loadError ? (
              <><p>图标列表加载失败，请重试。</p><Button variant="outline" onClick={() => void loadPanel()}>重新加载</Button></>
            ) : (
              <><LoaderCircle className="size-5 animate-spin motion-reduce:animate-none" aria-hidden="true" /><p>正在加载图标列表…</p></>
            )}
          </div>
        )}
      </DialogContent>
