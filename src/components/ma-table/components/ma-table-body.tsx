...
export function MaTableBody<T extends MaTableModel>({
  rows,
  columns,
  options,
  currentPage,
    ...
    if (column.type === 'selection')
      return (
        <Checkbox
          ...
          onCheckedChange={checked => onSelectionChange(row, checked === true, rowIndex)}
    ...
    if (column.type === 'expand')
      return (
        <Button
          ...
          variant="ghost"
          size="icon-xs"
          aria-label={expandedKeys.has(getRowKey(row, rowIndex)) ? '收起行' : '展开行'}
          onClick={() => onExpandChange(row, rowIndex)}
        >
  ...
  return (
    <TableBody className="[&_tr:last-child>td]:border-b">
      {rows.length ? (
        rows.map((row, rowIndex) => {
          const rowKey = getRowKey(row, rowIndex)
          const rowContext: MaTableCellContext<T> = {
            row,
            rowIndex,
            column: columns[0] ?? ({} as MaTableColumn<T>),
            value: undefined,
          }
          ...
          return (
            <React.Fragment key={rowKey}>
              <TableRow
                data-state={selectedKeys.has(rowKey) ? 'selected' : undefined}
                className={cn(
                  'border-border border-b transition-none hover:bg-muted/40 data-[state=selected]:bg-muted/50 [&:not(:last-child)>td]:border-b',
                  options.dense ? '[&>td]:h-10' : '[&>td]:h-12',
                  options.border && '*:last:border-e-0',
                  options.stripe && 'odd:bg-muted/90 odd:hover:bg-muted hover:bg-transparent',
                  onRowClick && 'cursor-pointer',
                  resolveRowClass(row, rowIndex, options.rowClassName),
                )}
                style={resolveRowStyle(row, rowIndex, options.rowStyle)}
                onClick={event => handleRowClick(event, row, rowIndex)}
              >
                {columns.map((column, columnIndex) => (
                  <TableCell
                    key={`${rowKey}-${String(column.prop ?? column.type ?? columnIndex)}`}
                    className={cn(
                      'align-middle whitespace-normal first:ps-(--frame-panel-header-px) last:pe-(--frame-panel-header-px)',
                      options.dense ? 'px-2 py-1.5' : 'px-3 py-2',
                    ...
                    style={{
                      ...
                      textAlign: column.align ?? options.columnAlign ?? 'left',
                    }}
                  >
              ...
              {expanded && columns.some(column => column.type === 'expand') && (
                <TableRow className="border-b transition-none">
                  <TableCell
                    colSpan={columns.length}
                    className="align-middle whitespace-normal first:ps-(--frame-panel-header-px) last:pe-(--frame-panel-header-px)"
                  >
                    {columns.find(column => column.type === 'expand')?.expandedRender?.(rowContext)}
                  </TableCell>
                </TableRow>
              )}
            </React.Fragment>
        ...
        <TableRow>
          <TableCell colSpan={Math.max(columns.length, 1)} className="py-6 text-center text-sm text-muted-foreground">
            {loading ? '加载中…' : (empty ?? options.emptyText ?? '暂无数据')}
          </TableCell>
        </TableRow>
      )}
    </TableBody>
