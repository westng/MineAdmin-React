  ...
  return JSON.stringify(
    {
      ...
      enabled: Boolean(options.api),
      auto: options.autoRequest,
...
function getOperationMinWidth<T extends MaModel>(
  ...
  const actionCount =
    mode === 'dropdown' ? 1 : Math.min(actions.length, fold) + (mode === 'auto' && actions.length > fold ? 1 : 0)
...
function MaProTableInner<T extends MaModel>(
  ...
  const updateOptions = React.useCallback(
    (nextOptions: Partial<MaProTableOptions<T>>) => {
      ...
      if (nextOptions.tableOptions)
        merged.tableOptions = { ...optionsRef.current.tableOptions, ...nextOptions.tableOptions }
      if (nextOptions.searchOptions)
        merged.searchOptions = { ...optionsRef.current.searchOptions, ...nextOptions.searchOptions }
      if (nextOptions.searchFormOptions)
        merged.searchFormOptions = { ...optionsRef.current.searchFormOptions, ...nextOptions.searchFormOptions }
      if (nextOptions.requestOptions) {
        requestSequenceRef.current += 1
  ...
  React.useEffect(() => {
    if (
      ...
      optionsRef.current.requestOptions?.autoRequest === false ||
      !optionsRef.current.requestOptions?.api
    )
      return
  ...
  const handlePageChange = React.useCallback(
    (nextPage: number, nextPageSize = pageSizeRef.current) => {
      ...
      const nextRows = optionsRef.current.selection?.crossPage
        ? [
            ...selectedRowsRef.current.filter(
              existing => !data.some(row => getSelectionKey(row) === getSelectionKey(existing)),
            ),
            ...rows,
          ]
        : rows
  ...
  const tableOptions = React.useMemo(
    () => ({
      ...
      pagination: {
        ...
        total:
          controlledData === undefined && options.requestOptions?.api
            ? total
            : (options.tableOptions?.pagination?.total ?? data.length),
        currentPage,
        pageSize,
  ...
  const proTableExpose = React.useMemo(() => {
    const expose: MaProTableExpose<T> = {
      ...
      setRequestParams: (params, requestNow = true) => {
        requestSequenceRef.current += 1
        if (optionsRef.current.requestOptions)
          updateOptions({
            requestOptions: {
              ...optionsRef.current.requestOptions,
              requestParams: { ...optionsRef.current.requestOptions.requestParams, ...params },
            },
          })
        if (requestNow) void requestData()
      },
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  {actionsToRender.map((action, index) => (
                    <React.Fragment key={action.name ?? `${String(action.text ?? 'operation')}-${index}`}>
            ...
            if (mode === 'dropdown' || (mode === 'auto' && visibleActions.length > fold)) {
              ...
              return (
                <div className="flex flex-wrap justify-end gap-1">
                  ...
                  {overflow.length > 0 && renderMenu(overflow)}
                </div>
              )
            }
  ...
  const showHeader = resolveVisible(
    headerConfig?.show,
    Boolean(header || headerConfig?.mainTitle || headerConfig?.subTitle),
  )
  const pluginToolbars = [...registeredToolbars.values()].filter(
    tool => resolveVisible(options.toolStates?.[tool.name], true) && (tool.show?.({ options }) ?? true),
  )
  const showToolbar = resolveVisible(
    options.toolbar,
    Boolean(
      toolbarCenter || toolbar || toolbarLeft || toolbarRight || beforeToolbar || afterToolbar || pluginToolbars.length,
    ),
  )
  const title = resolveText(headerConfig?.mainTitle, '数据列表')
  const subtitle = resolveText(headerConfig?.subTitle, '')
  const tableToolbarLeft = (
    <>
      {beforeToolbar}
      {toolbarLeft}
    </>
  ...
  const tableToolbarRight = (
    <>
      {toolbarRight}
      {!hasToolbarCenter && toolbarRight == null && (
        <Button type="button" variant="outline" size="sm" onClick={() => void requestData()} disabled={loading}>
          <RefreshCw className={cn('size-4', loading && 'animate-spin')} aria-hidden="true" />
          刷新
        </Button>
      )}
      {pluginToolbars.map(tool => {
  ...
  const selectionContent =
    selectedRows.length > 0 && options.selection ? (
      <div className="bg-muted/25 flex flex-col gap-3 border-b px-3 py-3 lg:flex-row lg:items-center lg:justify-between lg:px-4">
        <div className="flex min-w-0 flex-col gap-0.5">
          <span className="text-sm font-medium">
            {typeof options.selection.selectedText === 'function'
              ? options.selection.selectedText(selectedRows.length)
              : (options.selection.selectedText?.replace('{number}', String(selectedRows.length)) ??
                `已选择 ${selectedRows.length} 项`)}
          </span>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Button type="button" variant="ghost" size="sm" onClick={clearSelectedRows}>
            {resolveText(options.selection.clearText, '清除选择')}
          </Button>
        </div>
      </div>
    ) : undefined
  ...
  return (
    ...
    >
      {showHeader && (
        <FrameHeader className="min-w-0 flex-col items-start gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            error || showSearch ? (
              <>
                {error && (
                  <div
                    role="alert"
                    className="border-b border-destructive/40 bg-destructive/5 px-3 py-2 text-sm text-destructive"
                  >
                    {error}
                  </div>
                )}
                {showSearch && (
                  <>
                    <MaSearch
                      ...
                      formOptions={options.searchFormOptions}
