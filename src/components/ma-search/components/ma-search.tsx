...
function MaSearchInner<T extends MaModel>(
  {
    ...
    formOptions: initialFormOptions = emptyFormOptions,
    searchItems,
    items,
    className,
    children,
  ...
  ref: React.ForwardedRef<MaSearchExpose<T>>,
) {
  ...
  const [viewportColumns, setViewportColumns] = React.useState(() =>
    typeof window === 'undefined' ? 1 : getViewportColumns(initialOptions, window.innerWidth),
  )
  ...
  const updateFormOptions = React.useCallback(
    (nextOptions: MaFormOptions) => {
  ...
  const foldButtonVisible = React.useMemo(() => {
    const foldRows = options.foldRows ?? 2
    return (
      (options.foldButtonShow ?? true) &&
      currentItems.some(
        (item, index) => getFoldRow(currentItems, index, viewportColumns) >= foldRows && item.hide !== true,
      )
    )
  ...
  React.useImperativeHandle(
    ...
    () => ({
      ...
      getFold: () => folded,
      setSearchForm: form => {
        if (form === null) formRef.current?.setValues(null)
        else formRef.current?.setValues(form)
      },
      ...
      appendItem: item => {
        ...
        setCurrentItems(itemsRef.current)
      },
      removeItem: prop => {
        itemsRef.current = itemsRef.current.filter(
          item => resolveProp(item.prop, formRef.current?.getValues() ?? ({} as T)) !== prop,
        )
        setCurrentItems(itemsRef.current)
      },
      getItemByProp: prop =>
        itemsRef.current.find(item => resolveProp(item.prop, formRef.current?.getValues() ?? ({} as T)) === prop) ??
        null,
    ...
    <div className={cn('w-full rounded-lg border bg-card p-4', className)}>
      <MaForm
        ...
        options={{
          ...formOptions,
          layout: 'grid',
          grid: { ...formOptions.grid, columns: viewportColumns, gap: formOptions.grid?.gap ?? '1rem' },
        }}
        defaultValue={options.defaultValue as Partial<T>}
        onSubmit={handleSearch}
      >
        {children}
        <div
          className="flex flex-wrap items-center justify-start gap-2"
          style={{ gridColumn: '1 / -1', marginTop: formOptions.grid?.gap ?? '1rem' }}
        >
          {actions ?? (
            <>
              {beforeActions}
              <Button type="submit" {...searchButtonProps}>
                ...
                {readText(options.text?.searchBtn, '搜索')}
              </Button>
              <Button type="button" variant="outline" onClick={() => void handleReset()} {...resetButtonProps}>
                <RotateCcw className="size-4" aria-hidden="true" />
                {readText(options.text?.resetBtn, '重置')}
              </Button>
              {afterActions}
            </>
          )}
          {foldButtonVisible && (
            <Button type="button" variant="ghost" size="sm" aria-expanded={!folded} onClick={toggleFold}>
              {folded ? (
                <ChevronDown className="size-4" aria-hidden="true" />
              ) : (
                <ChevronUp className="size-4" aria-hidden="true" />
              )}
              {folded ? readText(options.text?.notFoldBtn, '展开') : readText(options.text?.isFoldBtn, '折叠')}
            </Button>
          )}
        </div>
      </MaForm>
...
export const MaSearch = React.forwardRef(MaSearchInner) as <T extends MaModel = MaModel>(
  props: MaSearchProps<T> & { ref?: React.ForwardedRef<MaSearchExpose<T>> },
) => React.ReactElement
