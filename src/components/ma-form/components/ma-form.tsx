...
import {
  ...
  FieldGroup,
  FieldLabel,
} from '@/components/reui/primitives/field'
...
function validateRule(
  ...
  value: unknown,
  model: MaModel,
): string | void | undefined | Promise<string | void> {
  const empty =
    value === undefined ||
    value === null ||
    value === '' ||
  ...
  if (rule.min !== undefined) {
    ...
    if (belowMinimum)
      return rule.message ?? (typeof value === 'number' ? `数值不能小于 ${rule.min}` : `长度不能少于 ${rule.min}`)
  }
  if (rule.max !== undefined) {
    ...
    if (aboveMaximum)
      return rule.message ?? (typeof value === 'number' ? `数值不能大于 ${rule.max}` : `长度不能超过 ${rule.max}`)
  }
  if (rule.pattern) {
...
function getColumnStyle<T extends MaModel>(item: MaFormItem<T>, columns: number): React.CSSProperties {
  ...
  return {
    ...
    marginInlineStart: safeOffset ? `${(safeOffset / columns) * 100}%` : undefined,
  }
}
...
function MaFormInner<T extends MaModel>(
  {
    modelValue,
    defaultValue,
    items: initialItems = emptyItems as MaFormItem<T>[],
  ...
  const getRulesForItem = React.useCallback(
    (item: MaFormItem<T>) => {
    ...
    >
      ...
      >
        {items.map((item, itemIndex) => {
          ...
          const context: MaFormRenderContext<T> = {
            item,
            formData: values,
            value,
            setValue: nextValue => {
              if (prop) setValue(prop, nextValue)
            },
          }
          ...
          return (
            <Field
              ...
              style={getColumnStyle(item, columns)}
            >
              {item.showLabel !== false && label !== undefined && label !== null && (
                <FieldLabel
                  className={itemProps.labelClassName}
                  htmlFor={prop}
                  style={
                    horizontalLabel && labelWidth
                      ? {
                          width: labelWidth,
                          flex: `0 0 ${labelWidth}`,
                          textAlign: labelPosition === 'right' ? 'right' : undefined,
                        }
                      : undefined
                  }
                >
                  {label}
                  {options.labelSuffix ?? ''}
                  {(itemProps.required || getRulesForItem(item).some(rule => rule.required)) && (
              ...
              <FieldContent>
                ...
                {itemProps.showMessage !== false && (
                  <FieldError id={errorId} errors={fieldError?.map(message => ({ message }))} />
                )}
                {item.itemSlots?.error?.(context)}
              </FieldContent>
              {item.children?.map((child, childIndex) => (
                <MaFormItemChildren
                  key={`${prop ?? itemIndex}-child-${childIndex}`}
                  item={child}
                  formData={values}
...
function MaFormItemChildren<T extends MaModel>({
  item,
  formData,
  setFieldValue,
}: {
  ...
  const context: MaFormRenderContext<T> = {
    item,
    formData,
    value: prop ? getPathValue(formData, prop) : undefined,
    setValue: value => {
      if (prop) setFieldValue(prop, value)
    },
  }
  return item.render && typeof item.render === 'function'
    ? React.createElement(item.render as React.ComponentType<MaFormRenderContext<T>>, context)
    : null
}

export const MaForm = React.forwardRef(MaFormInner) as <T extends MaModel = MaModel>(
  props: MaFormProps<T> & { ref?: React.ForwardedRef<MaFormExpose<T>> },
) => React.ReactElement
