...
基于官方 `sonner`，渲染层使用 shadcn `base-nova` registry 生成的 `components/reui/primitives/sonner.tsx`。项目封装保留 Sonner 的方法、参数类型和返回值，兼容原有调用。
...
```tsx
import { toast, useToast, useSonner } from '@/components/reui/use-toast'
...
`useToast()` 仍需在 `ToastProvider` 内调用，返回的 `toast` 同样提供全部方法。`useSonner()` 在组件中调用，返回官方通知状态；官方类型可以从 `@/components/reui/use-toast` 导入。无第二参数的 `toast(message)` 使用 Sonner 的普通通知样式。
