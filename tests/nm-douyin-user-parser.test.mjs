...
const user = { uid: '9876543210987654321', nickname: '测试达人', avatar_thumb: { url_list: ['https://images.example.test/avatar.png'] }, follower_count: 42 }
...
const fixture = {
  ...
  toast: () => {},
  http: {
    get: (url, config) => { calls.push({ method: 'get', url, config }); return respond(url, config) },
    post: async (url, data) => { calls.push({ method: 'post', url, data }); return { data: { code: 200 } } },
  },
}
window.addEventListener('mine:message', event => messages.push(event.detail))

const hasAllocation = existsSync(new URL('../src/modules/creator/allocation/views/components/CreatorAllocationForm.tsx', import.meta.url))
const hasCommission = existsSync(new URL('../src/modules/data/views/offlineCommission/index.tsx', import.meta.url))
const result = await build({
  stdin: {
    contents: [
      "export * from './src/components/nm-douyin-user-parser'",
      "export * from './src/components/nm-douyin-user-parser/utils/parser'",
      "export * from './src/components/nm-douyin-user-parser/api/douyin-user'",
      hasAllocation ? "export * from './src/modules/creator/allocation/views/components/CreatorAllocationForm'" : "",
      hasCommission ? "export { default as OfflineCommissionPage } from './src/modules/data/views/offlineCommission'" : "",
    ].join('\n'),
  ...
  plugins: [{
    ...
    setup(build) {
      ...
      build.onResolve({ filter: /^@\/utils\/http$/ }, () => ({ path: 'http', namespace: 'douyin-fixture' }))
      build.onResolve({ filter: /^@\/components\/common\/use-toast$/ }, () => ({ path: 'toast', namespace: 'douyin-fixture' }))
      build.onResolve({ filter: /^@\/components\/ma-pro-table$/ }, () => ({ path: 'table', namespace: 'douyin-fixture' }))
      build.onResolve({ filter: /\/hooks\/use-allocation-options$/ }, () => ({ path: 'options', namespace: 'douyin-fixture' }))
      build.onLoad({ filter: /.*/, namespace: 'douyin-fixture' }, args => ({ contents: sources[args.path] }))
    },
  }],
})
const compiled = { exports: {} }
new Function('module', 'exports', 'require', '__douyinFixture', result.outputFiles[0].text)(compiled, compiled.exports, require, fixture)
const { NmDouyinUserParser, extractSecUserId, normalizeDouyinUser, getDouyinUserProfile, CreatorAllocationForm, OfflineCommissionPage } = compiled.exports
...
async function mount(t, component, props) {
  ...
  const unmount = async () => { if (mounted) { await act(async () => root.unmount()); container.remove(); mounted = false } }
  t.after(unmount)
  await render(props)
...
test('主页链接支持查询参数、锚点、分享文字和无协议地址', () => {
  for (const link of [homepage, `  ${homepage}/?from=share#profile  `, homepage.replace('https://', ''), `分享达人主页：${homepage}，打开查看`]) {
    assert.equal(extractSecUserId(link), 'MS4wLjABAAAA-test_id')
  }
  assert.throws(() => extractSecUserId(' '), /请输入抖音用户主页链接/)
  for (const link of ['https://notdouyin.com/user/test', 'https://douyin.com.example.test/user/test', 'https://www.douyin.com/video/123', 'https://www.douyin.com/user/', 'https://example.test@www.douyin.com/user/test']) {
...
test('用户数据保留长 UID 和扩展字段，头像缺失与异常 UID 有明确处理', () => {
  ...
  for (const value of [undefined, null, {}, { uid: '' }]) assert.throws(() => normalizeDouyinUser(value), /未获取到用户 UID/)
  for (const uid of [0, -1, 'abc', {}, Number.MAX_SAFE_INTEGER + 1]) assert.throws(() => normalizeDouyinUser({ uid }), /UID 格式无效/)
})
...
test('输入框、解析和清空独立排列，使用 ReUI 输入框和按钮', async t => {
  ...
  assert.deepEqual([...group.children].map(element => element.tagName), ['INPUT', 'BUTTON', 'BUTTON'])
  assert.deepEqual([...group.querySelectorAll('button')].map(element => element.textContent), ['解析', '清空'])
...
test('空值和无效链接不会发送请求，输入和清空支持受控值', async t => {
  ...
  function ControlledInput() {
    ...
    return createElement(NmDouyinUserParser, { value, onChange: next => { changes.push(next); setValue(next) } })
  }
...
test('请求中防重复点击，成功后回调完整用户信息并恢复按钮', async t => {
  ...
  const parsed = []
  const { container } = await mount(t, NmDouyinUserParser, { defaultValue: homepage, dataHandle: value => parsed.push(value) })
  const parseButton = button(container, '解析')
  await act(async () => { parseButton.click(); parseButton.click() })
...
test('解析回车不会提交外层表单，中文输入法确认不会触发解析', async t => {
  let submits = 0
  function Form() {
    return createElement('form', { onSubmit: event => { event.preventDefault(); submits++ } }, createElement(NmDouyinUserParser, { defaultValue: homepage }))
  }
  const { container } = await mount(t, Form)
  const input = container.querySelector('input')
  await act(async () => input.dispatchEvent(new dom.KeyboardEvent('keydown', { key: 'Enter', isComposing: true, bubbles: true, cancelable: true })))
  assert.equal(calls.length, 0)
...
test('接口异常不回填，可以再次解析；缺少头像不影响成功', async t => {
  ...
  respond = async () => { throw { message: '解析服务暂时不可用' } }
  await click(button(container, '解析'))
...
test('达人分配表单关闭后取消请求，重新打开时不会回填旧用户', { skip: !hasAllocation && '当前仓库未包含达人业务模块' }, async t => {
  const request = deferred()
  respond = () => request.promise
  const changes = []
  const props = { open: true, editing: false, value: { aweme_name: '', aweme_id: '', performance_id: '', business_id: '' }, busy: false, onChange: value => changes.push(value), onClose() {}, onSubmit: async () => {} }
...
test('达人分配使用弹窗操作区触发表单校验，保存中禁止重复提交和关闭', { skip: !hasAllocation && '当前仓库未包含达人业务模块' }, async t => {
  const saved = []
  let closed = 0
  const props = { open: true, editing: false, value: { aweme_name: '', aweme_id: '', performance_id: '', business_id: '' }, busy: false, onChange() {}, onClose: () => { closed++ }, onSubmit: async value => { saved.push(value) } }
  const view = await mount(t, CreatorAllocationForm, props)
  const dialog = document.querySelector('[role="dialog"]')
  ...
  const valid = { ...props, value: { aweme_name: user.nickname, aweme_id: user.uid, performance_id: 'category', business_id: '7' } }
  await view.render({ ...valid, busy: true })
  assert.equal(button(footer, '保存中…').disabled, true)
  assert.equal(button(footer, '取消').disabled, true)
  await act(async () => dialog.querySelector('form').dispatchEvent(new dom.Event('submit', { bubbles: true, cancelable: true })))
...
test('达人分配新增表单自动回填并保留其他修改，提交不包含解析链接，编辑不展示解析', { skip: !hasAllocation && '当前仓库未包含达人业务模块' }, async t => {
  const saved = []
  const request = deferred()
  respond = () => request.promise
  function AllocationForm({ editing = false }) {
    const [value, onChange] = useState({ aweme_name: '', aweme_id: '', performance_id: 'category', leader_id: '', business_id: '7' })
    return createElement(CreatorAllocationForm, { open: true, editing, value, busy: false, onChange, onClose() {}, onSubmit: async values => { saved.push(values) } })
  }
  ...
  assert.deepEqual(saved[0], { aweme_name: user.nickname, aweme_id: user.uid, performance_id: 'category', leader_id: '2468', business_id: '7' })
  await view.render({ editing: true })
  assert.equal(document.querySelector('[aria-label="抖音用户主页链接"]'), null)
})

test('线下佣金表单回填后保留佣金字段与长 UID，编辑不展示解析', { skip: !hasCommission && '当前仓库未包含线下佣金业务模块' }, async t => {
  const { container } = await mount(t, OfflineCommissionPage)
  await click(button(container, '新增'))
  const dialog = document.querySelector('[role="dialog"]')
  const field = label => [...dialog.querySelectorAll('label')].find(node => node.querySelector('span')?.textContent === label)?.querySelector('input')
  ...
  const edit = fixture.table.schema.tableColumns.find(column => column.type === 'operation').operationConfigure.actions.find(action => action.name === 'edit')
