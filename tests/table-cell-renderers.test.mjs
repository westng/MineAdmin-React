  ...
  const result = await build({
    stdin: {
      contents: [
        ...
        "export { default as plugin, AvatarInfoCell, BadgeCell, ProductInfoCell } from './src/plugins/west/cell-enhance'",
...
function tableMarkup(h, columns, data, pro = false) {
  return renderToStaticMarkup(
    createElement(
      pro ? h.MaProTable : h.MaTable,
      pro
        ? {
            schema: { tableColumns: columns },
            data,
            options: { header: { show: false }, toolbar: false, tableOptions: { showPagination: false } },
          }
...
test('头像信息支持当前列值、按行配置和直接传值，缺失头像使用自定义文字或昵称首字', async () => {
  ...
  const direct = renderToStaticMarkup(
    createElement(h.AvatarInfoCell, {
      name: 'Alex Johnson',
      fallback: '用户',
      description: '创始人',
      badge: 'Pro',
      avatarSize: 'sm',
      badgeProps: { variant: 'info-light' },
      'aria-label': '用户信息',
    }),
  )
...
test('商品信息按嵌套字段显示图片、名称、商品 ID、状态和店铺描述', async () => {
  const h = await harness()
  h.plugin.install()
  const row = Object.freeze({
    id: 1,
    product: Object.freeze({ image: 'https://example.invalid/product.jpg', name: '春季连衣裙' }),
    productId: '9876543210987654321',
    shop: Object.freeze({ name: '示例抖店' }),
    status: '在售',
  })
  const column = {
    prop: 'product.name',
  ...
  const element = h.renderTableCell({ column, row, rowIndex: 0, value: row.product.name }, h.getTableCellRenderers())
  assert.equal(element.type, h.ProductInfoCell)
  assert.equal(element.props.image, row.product.image)
  assert.equal(element.props.name, row.product.name)
  assert.equal(element.props.productId, row.productId)
  assert.equal(element.props.badge, row.status)
  assert.equal(element.props.description, row.shop.name)
  assert.equal(element.props.fields, undefined)

  const html = tableMarkup(h, [column], [row])
  assert.match(html, /data-slot="product-image"/)
  assert.match(html, /src="https:\/\/example.invalid\/product.jpg"/)
  assert.match(html, /aria-label="查看商品大图"/)
  assert.match(html, /春季连衣裙/)
  assert.match(html, /data-slot="product-id"[^>]*>/)
  assert.match(html, /商品ID：9876543210987654321/)
  assert.match(html, /aria-label="复制商品ID"/)
  assert.match(html, /在售/)
  assert.match(html, /示例抖店/)
  assert.match(html, /bg-success\/10/)
})

test('商品信息缺图时显示商品首字，字段无效时回退到原始列值', async () => {
  const h = await harness()
  h.plugin.install()
  const productColumn = {
    prop: 'product.name',
    cellRenderTo: {
      ...
      props: {
        type: 'product-info',
        props: {
          fields: { image: 'product.image', name: 'product.name' },
        },
      },
    },
  }
  const html = tableMarkup(h, [productColumn], [{ id: 1, product: { name: '手机壳' } }])
  assert.match(html, /data-slot="product-image-fallback"[^>]*>手<\/span>/)
  assert.match(html, /手机壳/)
  assert.doesNotMatch(html, /data-slot="badge"/)

  const invalidConfig = {
    name: 'west/cell-enhance',
    props: { type: 'product-info', props: { imageSize: 'invalid' } },
  }
  const fallback = tableMarkup(h, [{ prop: 'name', cellRenderTo: invalidConfig }], [{ id: 2, name: '原始商品列值' }])
  assert.match(fallback, />原始商品列值<\/td>/)
  assert.doesNotMatch(fallback, /data-slot="product-image"/)
})
...
test('MaTable 与 MaProTable 通过 cellRenderTo 同时支持 dictName 和 options 数组', async () => {
  ...
  for (const [config, color] of [
    [badgeConfig, /bg-success\/10/],
    [dictionaryConfig, /bg-primary\/10/],
  ]) {
      ...
      options: [
        {
          ...
          style: { color: 'blue' },
          render: createElement('a', { href: '#detail' }),
        },
...
test('错误渲染配置回退到原值，不会把错误属性传给 Badge', async () => {
  ...
  for (const props of [
    ...
    const html = tableMarkup(
      h,
      [{ prop: 'status', cellRenderTo: { name: 'west/cell-enhance', props } }],
      [{ id: 1, status: 3 }],
    )
...
test('字典按分类 code 取值，缺失分类和未知值保留原值', async () => {
  ...
  for (const pro of [false, true]) {
    const columns = dictName => [
      {
        prop: 'status',
        cellRenderTo: {
          name: 'west/cell-enhance',
          props: { type: 'badge', props: { dictName } },
        },
      },
    ]
  ...
  const formatted = renderToStaticMarkup(
    createElement(h.BadgeCell, {
      value: 9,
      dictName: 'missing-category',
      formatValue: value => `未知 ${value}`,
    }),
  )
