...
async function readFunctions(path, names) {
  const source = ts.createSourceFile(String(path), await readFile(path, 'utf8'), ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX)
...
const pagePath = new URL('../src/modules/base/permission/menu/views/components/permission-menu-page.tsx', import.meta.url)
const source = [
...
function menu() {
  return {
    id: 632, parent_id: 0, name: 'feishu:connection', path: '/feishu/connection',
    component: 'feishu/connection/views/index', status: 1,
...
test('新增按钮补齐类型与空翻译键，转换过程不修改表单状态', () => {
  ...
  assert.deepEqual(toPayload(form).btnPermission.at(-1), {
    title: '测试飞书连接', code: 'feishu:connection:test', type: 'B', i18n: '',
  })
...
test('只删除选中的按钮，显式清空时仍提交空数组', () => {
  ...
  assert.deepEqual(toPayload(form).btnPermission.map(button => button.id), [702])
  form.btnPermission = []
