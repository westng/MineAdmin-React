...
const result = await build({
  stdin: {
    contents: [
      "export { default as MenuPage } from './src/modules/base/permission/menu/views/index'",
  ...
  plugins: [
    {
      ...
      setup(build) {
        build.onResolve({ filter: /^@\/hooks\/framework\/use-permission$/ }, () => ({
          path: 'permissions',
          namespace: 'permissions-fixture',
        }))
        build.onLoad({ filter: /.*/, namespace: 'permissions-fixture' }, () => ({
          contents: 'export const PermissionGate = ({ children }) => children;',
        }))
        build.onResolve({ filter: /^(menu-test-api|@\/modules\/base\/permission\/menu\/api\/menu)$/ }, () => ({
          path: 'api',
          namespace: 'menu-fixture',
        }))
        build.onResolve({ filter: /^@\/components\/(?:ma-icon-picker|ma-icon)$/ }, () => ({
          path: 'icons',
          namespace: 'menu-fixture',
        }))
        build.onLoad({ filter: /.*/, namespace: 'menu-fixture' }, ({ path }) => ({
          contents:
            path === 'api'
              ...
              : `export const MaIcon = () => null; export const MaIconPicker = () => null`,
        }))
      },
    },
  ],
...
const { MenuPage, MenuCascader, HeaderActionsSetterContext, RuntimeContext, setApi } = compiled.exports
      ...
      children: [
        {
          ...
          status: 1,
          component: 'base/permission/user/views/index',
          meta: { type: 'M', title: '用户管理' },
          children: [
            {
              id: 101,
              parent_id: 10,
              ...
              status: 1,
              meta: { type: 'B', title: '查看用户', i18n: 'user.view' },
            },
            {
              id: 102,
              parent_id: 10,
...
function Header({ children }) {
  ...
  return createElement(
    ...
    { value: setActions },
    createElement('header', null, actions),
    children,
  )
}
  ...
  await act(async () =>
    root.render(
      createElement(
        ...
        { value: { components: { has: () => true } } },
        createElement(Header, null, component),
      ),
    ),
  )
  await act(async () => {
...
test('父菜单增删按钮后刷新树和表单，重复保存保留新按钮 ID', async t => {
  ...
  withButton[0].children[0].children.push({
    id: 103,
    parent_id: 10,
    name: 'permission:user:export',
    status: 1,
    meta: { type: 'B', title: '导出用户' },
  })
  const requests = []
  setApi({
  ...
  await click(button(panel, '保存'))
  assert.deepEqual(
    requests[2].payload.btnPermission.map(item => item.id),
    [101, 102],
  )
...
test('父级菜单单选保留完整 ID，不把两位数菜单 ID 截断', async t => {
  ...
  await click(
    [...document.querySelectorAll('[data-slot="cascader-item"]')].find(node => node.textContent.trim() === '目标菜单'),
  )
