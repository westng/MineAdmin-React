...
export const dialog = <MaDialog handle={handle} height={560} maxHeight="80dvh" onOpenChange={(_open, details) => { details.cancel(); details.preventUnmountOnClose() }}>
  {({ payload }) => payload?.title}
</MaDialog>
export const drawer = <MaDrawer handle={handle} popupProps={{ initialFocus: false }} portalProps={{ keepMounted: true }}>
  {({ payload }) => payload?.title}
</MaDrawer>
...
export const formItems: MaFormItem[] = [
  ...
  { prop: 'tags', render: 'Select', renderProps: { multiple: true, options: [{ label: '一', value: 1 }], onValueChange: (_value, details) => details.cancel(), popupProps: { portalProps: { container: document.body }, positionerProps: { sideOffset: 8 } } } },
  { prop: 'count', render: 'InputNumber', renderProps: { min: 0, step: 2, onValueChange: (_value, details) => details.reason } },
  { prop: 'enabled', render: 'Switch', renderProps: { onCheckedChange: (_checked, details) => details.cancel() } },
  { prop: 'date', render: 'DatePicker', renderProps: { mode: 'range', valueFormat: 'yyyy-MM-dd', calendarProps: { numberOfMonths: 2 } } },
  { prop: 'time', render: 'TimePicker', renderProps: { minuteStep: 15, onValueChange: (_value, details) => details.cancel() } },
...
export const tableOptions: MaTableOptions<Row> = {
  ...
  dataGridProps: { tableLayout: { columnsResizable: true, headerSticky: true }, onRowClick: row => row.name, onCellsChange: details => details.changes },
...
export const invalidTable: MaTableOptions<Row> = { tableOptions: { state: { pagination: { pageIndex: 0, pageSize: 10 } } } }
