...
const result = await build({
  stdin: {
    contents: [
      ...
      hasImportExport ? "export { default as plugin } from './src/plugins/west/importExportPro'" : "export const plugin = null",
      "export { ToastContext } from './src/components/common/toast-context'",
    ].join('\n'),
  ...
  plugins: [{
    ...
    setup(build) {
      build.onResolve({ filter: /^@\/utils\/http$/ }, () => ({ path: 'http', namespace: 'fixture' }))
      build.onResolve({ filter: /^\.\/views\/ImportExportTaskPage$/ }, () => ({ path: 'task-page', namespace: 'fixture' }))
      build.onLoad({ filter: /.*/, namespace: 'fixture' }, ({ path }) => ({ contents: path === 'http' ? 'export default __toolbarHttpFixture' : 'export default function TaskPage() { return null }' }))
    },
  }],
})
const compiled = { exports: {} }
new Function('module', 'exports', 'require', '__toolbarHttpFixture', result.outputFiles[0].text)(compiled, compiled.exports, require, http)
const { MaProTable, getProTableToolbars, registerProTableToolbar, removeProTableToolbar, plugin, ToastContext } = compiled.exports
...
async function mount(t, props) {
  ...
  const render = async nextProps => {
    await act(async () => root.render(createElement(ToastContext.Provider, { value: { toast: () => {} } }, createElement(MaProTable, { data: emptyRows, ...nextProps }))))
  }
...
beforeEach(async () => {
  await act(async () => { for (const name of getProTableToolbars().keys()) removeProTableToolbar(name) })
  calls.length = 0
})
...
test('注册工具默认进入右侧并按 order 排序，保留左侧动作和刷新', async t => {
  ...
  assert.deepEqual([...toolbar.lastElementChild.querySelectorAll('button')].map(node => node.textContent), ['刷新', '前工具', '后工具'])
})
...
test('同名工具替换不重复，旧清理函数不会移除新注册，注销会更新已挂载表格', async t => {
  ...
  let removeNew
  await act(async () => { removeNew = registerProTableToolbar({ name: 'tool', render: () => createElement('button', null, '新工具') }) })
...
test('插件 setup 自动注入工具，重复启动不重复显示，toolStates 支持布尔和函数开关', { skip: !hasImportExport && '当前仓库未安装本地导入导出插件' }, async t => {
  const ref = createRef()
  const view = await mount(t, { ref, options: allocationOptions() })
  assert.equal(button(view.container, '导入'), undefined)
  await act(async () => { plugin.hooks.setup(); plugin.hooks.setup() })
  ...
  await act(async () => ref.current.setProTableOptions({ toolStates: { [importTool]: false, [exportTool]: () => true } }))
  assert.equal(button(view.container, '导入'), undefined)
  assert.ok(button(view.container, '导出'))
  await view.render({ ref, options: allocationOptions({ toolStates: { [importTool]: () => true, [exportTool]: false } }) })
  assert.ok(button(view.container, '导入'))
  assert.equal(button(view.container, '导出'), undefined)
})

test('无导入导出接口的静态表格不注入按钮，toolbar=false 可关闭全部工具', { skip: !hasImportExport && '当前仓库未安装本地导入导出插件' }, async t => {
  plugin.hooks.setup()
  const view = await mount(t, { options: options() })
...
test('自动导入沿用当前表格配置，未传外部 ref 也能在导入后刷新', { skip: !hasImportExport && '当前仓库未安装本地导入导出插件' }, async t => {
  plugin.hooks.setup()
  const requests = []
  const view = await mount(t, { options: allocationOptions({ requestOptions: { autoRequest: false, api: params => { requests.push(params); return { list: [], total: 0 } } } }) })
...
test('模板下载沿用默认 downloadTemplate 路径并受配置权限控制', { skip: !hasImportExport && '当前仓库未安装本地导入导出插件' }, async t => {
  plugin.hooks.setup()
  const list = '/admin/creator/allocation/list'
  const view = await mount(t, { options: allocationOptions({ importExportPro: { api: { list }, permissions: { template: false } } }) })
  await click(button(view.container, '导入'))
  ...
  try { await click(button(document, '下载导入模板')) }
  finally { dom.HTMLAnchorElement.prototype.click = originalClick }
  assert.equal(calls.length, 1)
  assert.equal(calls[0].url, '/admin/creator/allocation/downloadTemplate')
  assert.equal(calls[0].config.responseType, 'blob')
})

test('自动导出通过同一列表地址推导字段和提交接口', { skip: !hasImportExport && '当前仓库未安装本地导入导出插件' }, async t => {
  plugin.hooks.setup()
