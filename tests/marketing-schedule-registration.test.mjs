...
const {
  scheduleToEvents,
  getSchedulePeriods,
  scheduleWithUpdatedCalendarRange,
  scheduleSegmentRange,
  scheduleTimelineSlices,
  buildEventIndex,
  SchedulePhaseTimeline,
  validateScheduleTimes,
  scheduleToForm,
  scheduleFormPayload,
  registrationStatusLabel,
  operationChanges,
  ScheduleRegistrationBadge,
  emptyForm,
  fallbackScheduleDictionaries,
  filterSchedulesByCreator,
} = module.exports
...
const schedule = {
  ...
  business_type: 'official_activity',
  schedule_status: 'planned',
  created_by: 10,
  can_update: true,
  registration_start_at: '2026-08-25T00:00:00',
  registration_end_at: '2026-08-30T23:59:59',
  campaign_start_at: '2026-09-08T00:00:00',
  campaign_end_at: '2026-09-15T23:59:59',
}
...
test('dragging the combined event moves all four times by the same offset', () => {
  const moved = scheduleWithUpdatedCalendarRange(
    schedule,
  ...
  assert.equal(
    scheduleWithUpdatedCalendarRange(schedule, new Date('2026-08-26T00:00:00'), new Date('2026-09-15T23:59:59')),
    null,
  )
  const legacy = { ...schedule, registration_start_at: null, registration_end_at: null }
  const movedLegacy = scheduleWithUpdatedCalendarRange(
    legacy,
...
test('form and drag validation reject missing, invalid, or reversed phase times', () => {
  ...
  for (const patch of [
    { registration_start_at: null },
    { registration_end_at: null },
    { registration_end_at: 'invalid' },
    { registration_end_at: '2026-08-24T00:00:00' },
    { campaign_start_at: 'invalid' },
    { campaign_end_at: '2026-09-07T00:00:00' },
  ])
    assert.equal(typeof validateScheduleTimes({ ...schedule, ...patch }), 'string')
  assert.equal(
    validateScheduleTimes({
      ...schedule,
      registration_end_at: schedule.registration_start_at,
      campaign_end_at: schedule.campaign_start_at,
    }),
    null,
  )
})
...
test('touching, overlapping, and coinciding phases remain a single editable event', () => {
  for (const patch of [
    ...
    assert.deepEqual(
      scheduleToEvents(record).map(event => event.id),
      ['42'],
    )
  }
})
...
test('registration has two business states and legacy empty values display as unregistered', () => {
  ...
  assert.deepEqual(
    operationChanges(
      { changed_fields: { registration_status: { before: null, after: 'registered' } } },
      fallbackScheduleDictionaries,
    ),
  ...
  for (const business_type of ['seeding', 'marketing_node', 'affiliate_live', 'affiliate_short_video']) {
    assert.equal(
      renderToStaticMarkup(
        createElement(ScheduleRegistrationBadge, {
          schedule: { ...schedule, business_type, registration_status: 'registered' },
        }),
      ),
      '',
    )
...
test('the month layout packs one lane per record even when registration and activity overlap', () => {
  const record = {
    ...schedule,
    registration_start_at: '2026-08-13T16:00:00',
    registration_end_at: '2026-09-25T23:59:00',
    campaign_start_at: '2026-08-20T00:00:00',
    campaign_end_at: '2026-09-25T23:59:59',
  }
  const index = buildEventIndex(
    scheduleToEvents(record),
    { start: new Date('2026-08-31T00:00:00'), end: new Date('2026-09-28T00:00:00') },
    { timeZone: 'Asia/Shanghai', weekStartsOn: 1 },
  )
...
test('the single timeline distinguishes registration, overlap, activity, and gaps in date order', () => {
  ...
  const record = {
    ...schedule,
    registration_start_at: '2026-09-01T00:00:00',
    registration_end_at: '2026-09-03T00:00:00',
    campaign_start_at: '2026-09-02T00:00:00',
    campaign_end_at: '2026-09-05T00:00:00',
  }
  assert.deepEqual(scheduleTimelineSlices(record, range), [
    { phase: 'registration', left: 0, width: 25 },
    { phase: 'overlap', left: 25, width: 25 },
    { phase: 'campaign', left: 50, width: 50 },
  ])
  assert.deepEqual(
    scheduleTimelineSlices(
      { ...record, registration_end_at: '2026-09-02T00:00:00', campaign_start_at: '2026-09-04T00:00:00' },
      range,
    ),
    [
      { phase: 'registration', left: 0, width: 25 },
      { phase: 'gap', left: 25, width: 50 },
      { phase: 'campaign', left: 75, width: 25 },
    ],
  )
  assert.deepEqual(
    scheduleTimelineSlices(record, { start: new Date('2026-09-02T00:00:00'), end: new Date('2026-09-04T00:00:00') }),
    [
      { phase: 'overlap', left: 0, width: 50 },
      { phase: 'campaign', left: 50, width: 50 },
    ],
  )
  assert.deepEqual(
    scheduleTimelineSlices(schedule, { start: new Date('2026-09-01T00:00:00'), end: new Date('2026-09-07T00:00:00') }),
    [],
  )
})
...
test('disjoint and identical phases both stay on a single lane', () => {
  for (const record of [
    schedule,
    { ...schedule, registration_start_at: schedule.campaign_start_at, registration_end_at: schedule.campaign_end_at },
  ]) {
    const index = buildEventIndex(
      scheduleToEvents(record),
      { start: new Date('2026-08-24T00:00:00'), end: new Date('2026-09-21T00:00:00') },
      { timeZone: 'Asia/Shanghai', weekStartsOn: 1 },
    )
    assert.equal(index.occurrences.length, 1)
...
test('invalid or missing dates do not create phantom calendar events or invalid timeline widths', () => {
  ...
  const record = {
    ...schedule,
    registration_start_at: schedule.campaign_start_at,
    registration_end_at: schedule.campaign_start_at,
    campaign_end_at: schedule.campaign_start_at,
  }
  const slices = scheduleTimelineSlices(record, {
...
test('status and phase text stays readable in both themes, including both overlap stripe colors', () => {
  ...
  for (const theme of ['light', 'dark']) {
    for (const appearance of [
      ...Object.values(scheduleStatusAppearance),
      ...Object.values(scheduleRegistrationAppearance),
      ...Object.values(schedulePhaseAppearance),
    ]) {
      assertReadable(appearance[theme].foreground, appearance[theme].background)
    }
    for (const phase of ['registration', 'campaign']) {
      assertReadable(
        schedulePhaseAppearance.overlap[theme].foreground,
        schedulePhaseAppearance[phase][theme].background,
      )
    }
  }
})
