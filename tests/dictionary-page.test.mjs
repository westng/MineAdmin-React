...
const result = await build({
  ...
  plugins: [{
    ...
    setup(build) {
      build.onResolve({ filter: /^(dictionary-test-api|@\/utils\/http)$/ }, () => ({ path: 'api', namespace: 'dictionary-fixture' }))
      build.onResolve({ filter: /^@\/store\/modules\/useUserStore$/ }, () => ({ path: 'user', namespace: 'dictionary-fixture' }))
      build.onLoad({ filter: /.*/, namespace: 'dictionary-fixture' }, ({ path }) => ({
        contents: path === 'api'
          ...
          : `import { create } from 'zustand'; export const useUserStore = create(() => ({ permissions: ['dataCenter:dictionary:save', 'dataCenter:dictionary:update', 'dataCenter:dictionary:delete'], roles: [] }))`,
        resolveDir: fileURLToPath(new URL('../', import.meta.url)),
      }))
    },
  }],
...
const types = [{ id: 1, name: '分类一', code: 'first', status: 1 }, { id: 2, name: '分类二', code: 'second', status: 1 }]
const row = typeId => ({ id: typeId * 10, type_id: typeId, label: `分类${typeId}条目`, value: '1', code: `item-${typeId}`, i18n_scope: 1, status: 1, sort: 0 })
...
async function mount(t) {
  ...
  const errors = []
  const context = { toast: message => { errors.push(message) } }
  t.after(async () => { await act(async () => root.unmount()); container.remove() })
  await act(async () => root.render(createElement(ToastContext.Provider, { value: context }, createElement(DictionaryPage))))
  return { container, errors }
}

function button(container, text) {
  return [...container.querySelectorAll('button')].find(node => node.textContent.trim() === text || node.getAttribute('aria-label') === text)
}
...
test('字典页面打开、选择、编辑弹窗和切换分类后保留正确的查询参数', async t => {
  ...
  assert.equal(requests.at(-1).type_id, 1)
  assert.match(container.querySelector('tbody').textContent, /分类1条目/)
  const secondType = [...container.querySelectorAll('aside [role="button"]')].find(node => node.textContent.includes('分类二'))
