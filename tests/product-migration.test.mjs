...
const bundled = await build({
  stdin: {
    contents: [
      "export { default as ProductListPage } from './src/modules/product/product-list/views/components/product-list-page'",
  ...
  plugins: [
    {
      ...
      setup(build) {
        const sources = {
          ...
          toast: 'export function useToast() { return { toast: (...args) => fixture.messages.push(args) } }',
          ...
          export function MaDrawer(props) { fixture.drawer = props; return createElement('div', {role:'dialog'}, props.title, props.children, props.footer) }`,
          dialog:
            'export function Dialog() { return null }; export function DialogContent() {}; export function DialogHeader() {}; export function DialogTitle() {}; export function DialogDescription() {}',
          product:
...
test('迁移按钮打开当前商品抽屉，提交仅包含该商品', async () => {
  await renderPage()
  assert.equal(fixture.table.schema.tableColumns[0].type, 'selection')
  assert.equal(fixture.table.schema.tableColumns.at(-1).fixed, 'right')
  assert.equal(fixture.table.options.tableOptions.dataGridProps.tableLayout.columnsPinnable, true)
